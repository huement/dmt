...
class Circle
{
    ...
    public function __construct($imagine, $themeName, $themeType, $shuffle, $texture)
    {
        ...
        TUI::Speaks("Get the colors & theme data");
    ...
    public function backgroundTexture()
    {
        ...
        $tempString   = FileController::randomTxtString(5);
        ...
        FileController::remove_file($tempFile);
    ...
    public function makeCircleInline()
    {
        ...
        foreach ($this->theme["colors"] as $color) {
            ...
            $circleResults[] = ["x" => $startX, "y" => $Y, "size" => $sizes, "color" => $color];
    ...
    public function makeCircleRows($colorMode = "random")
    {
        ...
        while ($tenCount <= $tenMax) {
            ...
            while ($rowCount <= $rowMax) {
                ...
                if ($colorMode === "random") {
                    if ($randChance >= intval(round($tcs * 0.5))) {
                        $mainColor = $this->theme["colors"][$randColor];
                    }
    ...
    public function coolCompounder($data)
    {
        ...
        foreach ($this->theme["colors"] as $color) {
            ...
            $results[] = ["x" => $loc["x"], "y" => $loc["y"], "size" => $sizes, "color" => $color];
        }
        ...
        foreach ($this->theme["colors"] as $color) {
            ...
            if (($variation % 2) === 0) {
                ...
                $loc  = $this->randomCoordinate();
                $mR[] = ["x" => $loc["x"], "y" => $loc["y"], "size" => $sizes, "color" => $color, "fill" => $fill, "line" => $line];
            }
        }
    ...
    public function makeRainbow($pos = true, $cloud = true)
    {
        ...
        if ($this->theme["LoD"] !== "light" && $cloud === true) {
            $this->cloudMaker("A");
            $this->cloudMaker("B");
        } elseif ($this->theme["LoD"] === "light" && $cloud === true) {
    ...
    public function cloudMaker($key, $inverse = false)
    {
        ...
        if ($inverse === false) {
            $cloudShadow = ColorController::getDarker("FFFFFF", 0.30);
            $cloudColor  = "FFFFFF";
        } else {
            $cloudShadow = ColorController::getDarker($this->theme["bg"], 0.25);
            $cloudColor  = ColorController::getDarker($this->theme["bg"], 0.20);
        }
    ...
    public function relativeRingSize($key)
    {
        ...
        return ["size" => $size, "line" => $thick, "fill" => $fill];
    }
