...
// use App\Commands\Controller\TUI;
// use App\Commands\Controller\SixteenController;
// use App\Commands\Controller\WallpaperController;
use App\Console\TUI;
...
class Rectangle
{
     ...
     * @return void
     */
    public function computeSpecCoords($tclrs = 8, $item_width = 100, $item_height = 100, $colors = [], $singleMode = true, $rowTwo = false)
    {
    ...
    public function caveLoop($image, $wData, $full = false)
    {
        ...
        $trData = ["roundness" => 0, "style" => "filled"];
        $brData = ["roundness" => 0, "style" => "filled"];
    ...
    public function makeCaveOfColors($image, $themeName = false, $themeType = false, $fillScreen = false, $shuffle = false, $texture = false)
    {
        ...
        if (self::DEBUG_MODE !== false) {
            $data = ["name" => $themeName, "type" => $themeType, "fillScreen" => $fillScreen, "shuffle" => $shuffle, "texture" => $texture];
            TUI::printDebug($data);
            //exit;
        }
    ...
    public function makeSpecRectangle($image, $themeName = false, $themeType = false, $singleDouble = true, $shuffle = false, $texture = false)
    {
        ...
        if ($singleDouble !== true && isset($wData["grays"])) {
            $string = "fullSpec_" . $wData["themeName"] . "_";
        }
    ...
    {
        $allColors   = SixteenController::spectrumSorter($theme, false, true);
        ...
        $rd = ["start_x" => 0, "stop_x" => $indiv_rect_width];
        ...
        foreach ($spectrum as $color) {
            ...
            $last_stop  = $rd["stop_x"];
            $rd = ["start_x" => $last_stop, "stop_x" => intval(round($last_stop + $indiv_rect_width))];
        }
